...
- Comments explain intent, constraints, and non-obvious decisions — not mechanics or code that is already self-explanatory.
- Exported functions in `db/` and `src/lib/` include TSDoc/JSDoc for their purpose, parameters, and return values.
- Reusable Astro components document their `Props` interface and public contract.
- TypeScript uses two-space indentation, semicolons, single quotes, and consistent multiline formatting; ESLint checks whitespace formatting rules.
